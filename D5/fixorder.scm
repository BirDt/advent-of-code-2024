...
(define invalid-lists (compress (map (complement test-validity) page-lists) page-lists))

(define (get-middle lst)
  (list-ref lst (/ (- (length lst) 1) 2)))

(define (valid-pos? x y)
  (test-validity (list x y)))

(display (apply + (map get-middle (map (lambda (x) (sort x valid-pos?)) invalid-lists))))
